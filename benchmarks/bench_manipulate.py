...
class BroadcastArrays(Benchmark):
    ...
    def time_broadcast_arrays_th(self, shape, ndtype):
        c(np.broadcast_arrays)(self.xarg, np.ones(1))
...
class BroadcastArraysTo(Benchmark):
    ...
    def time_broadcast_to_th(self, size, ndtype):
        c(np.broadcast_to)(self.xarg, (size, size))
...
class ConcatenateStackArrays(Benchmark):
    ...
    def time_concatenate_ax0_th(self, size, narrays, ndtype):
        c(np.concatenate)(self.xarg, axis=0)

    def time_concatenate_ax1(self, size, narrays, ndtype):
        np.concatenate(self.xarg, axis=1)

    def time_concatenate_ax1_th(self, size, narrays, ndtype):
        c(np.concatenate)(self.xarg, axis=1)

    def time_stack_ax0(self, size, narrays, ndtype):
        np.stack(self.xarg, axis=0)

    def time_stack_ax0_th(self, size, narrays, ndtype):
        c(np.stack)(self.xarg, axis=0)

    def time_stack_ax1(self, size, narrays, ndtype):
        np.stack(self.xarg, axis=1)

    def time_stack_ax1_th(self, size, narrays, ndtype):
        c(np.stack)(self.xarg, axis=1)
...
class DimsManipulations(Benchmark):
    ...
    def time_expand_dims_th(self, shape):
        c(np.expand_dims)(self.xarg, axis=1)

    def time_expand_dims_neg(self, shape):
        np.expand_dims(self.xarg, axis=-1)

    def time_expand_dims_neg_th(self, shape):
        c(np.expand_dims)(self.xarg, axis=-1)

    def time_squeeze_dims(self, shape):
        np.squeeze(self.xarg)

    def time_squeeze_dims_th(self, shape):
        c(np.squeeze)(self.xarg)

    def time_flip_all(self, shape):
        np.flip(self.xarg, axis=None)

    def time_flip_all_th(self, shape):
        c(np.flip)(self.xarg, axis=None)

    def time_flip_one(self, shape):
        np.flip(self.xarg, axis=1)

    def time_flip_one_th(self, shape):
        c(np.flip)(self.xarg, axis=1)

    def time_flip_neg(self, shape):
        np.flip(self.xarg, axis=-1)

    def time_flip_neg_th(self, shape):
        c(np.flip)(self.xarg, axis=-1)

    def time_moveaxis(self, shape):
        np.moveaxis(self.xarg, [0, 1], [-1, -2])

    def time_moveaxis_th(self, shape):
        c(np.moveaxis)(self.xarg, [0, 1], [-1, -2])

    def time_roll(self, shape):
        np.roll(self.xarg, 3)

    def time_roll_th(self, shape):
        c(np.roll)(self.xarg, 3)

    def time_reshape(self, shape):
        np.reshape(self.xarg, self.reshaped)


    def time_reshape_th(self, shape):
        c(np.reshape)(self.xarg, self.reshaped)
