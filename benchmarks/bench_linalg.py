...
class Eindot(Benchmark):
    ...
    def time_dot_a_b_th(self):
        c(np.dot)(self.a, self.b)

    def time_dot_d_dot_b_c(self):
        np.dot(self.d, np.dot(self.b, self.c))

    def time_dot_d_dot_b_c_th(self):
        c(np.dot)(self.d, np.dot(self.b, self.c))

    def time_dot_trans_a_at(self):
        np.dot(self.a, self.at)

    def time_dot_trans_a_at_th(self):
        c(np.dot)(self.a, self.at)

    def time_dot_trans_a_atc(self):
        np.dot(self.a, self.atc)

    def time_dot_trans_a_atc_th(self):
        c(np.dot)(self.a, self.atc)

    def time_dot_trans_at_a(self):
        np.dot(self.at, self.a)

    def time_dot_trans_at_a_th(self):
        c(np.dot)(self.at, self.a)

    def time_dot_trans_atc_a(self):
        np.dot(self.atc, self.a)

    def time_dot_trans_atc_a_th(self):
        c(np.dot)(self.atc, self.a)

    def time_einsum_i_ij_j(self):
        np.einsum('i,ij,j', self.d, self.b, self.c)

    def time_einsum_i_ij_j_th(self):
        c(np.einsum)('i,ij,j', self.d, self.b, self.c)

    def time_einsum_ij_jk_a_b(self):
        np.einsum('ij,jk', self.a, self.b)

    def time_einsum_ij_jk_a_b_th(self):
        c(np.einsum)('ij,jk', self.a, self.b)

    def time_einsum_ijk_jil_kl(self):
        np.einsum('ijk,jil->kl', self.a3, self.b3)

    def time_einsum_ijk_jil_kl_th(self):
        c(np.einsum)('ijk,jil->kl', self.a3, self.b3)
    ...
    def time_inner_trans_a_ac_th(self):
        c(np.inner)(self.a, self.ac)

    def time_matmul_a_b(self):
        np.matmul(self.a, self.b)

    def time_matmul_a_b_th(self):
        c(np.matmul)(self.a, self.b)

    def time_matmul_d_matmul_b_c(self):
        np.matmul(self.d, np.matmul(self.b, self.c))

    def time_matmul_d_matmul_b_c_th(self):
        c(np.matmul)(self.d, np.matmul(self.b, self.c))

    def time_matmul_trans_a_at(self):
        np.matmul(self.a, self.at)

    def time_matmul_trans_a_at_th(self):
        c(np.matmul)(self.a, self.at)

    def time_matmul_trans_a_atc(self):
        np.matmul(self.a, self.atc)

    def time_matmul_trans_a_atc_th(self):
        c(np.matmul)(self.a, self.atc)

    def time_matmul_trans_at_a(self):
        np.matmul(self.at, self.a)

    def time_matmul_trans_at_a_th(self):
        c(np.matmul)(self.at, self.a)

    def time_matmul_trans_atc_a(self):
        np.matmul(self.atc, self.a)

    def time_matmul_trans_atc_a_th(self):
        c(np.matmul)(self.atc, self.a)

    def time_tensordot_a_b_axes_1_0_0_1(self):
        np.tensordot(self.a3, self.b3, axes=([1, 0], [0, 1]))

    def time_tensordot_a_b_axes_1_0_0_1_th(self):
        c(np.tensordot)(self.a3, self.b3, axes=([1, 0], [0, 1]))
...
class Linalg(Benchmark):
    ...
    def time_svd_th(self, typename):
        c(np.linalg.svd)(self.a)

    def time_pinv(self, typename):
        np.linalg.pinv(self.a)

    def time_pinv_th(self, typename):
        c(np.linalg.pinv)(self.a)

    def time_det(self, typename):
        np.linalg.det(self.a)

    def time_det_th(self, typename):
        c(np.linalg.det)(self.a)
...
class LinalgNorm(Benchmark):
    ...
    def time_norm_th(self, typename):
        c(np.linalg.norm)(self.a)
...
class LinalgSmallArrays(Benchmark):
    ...
    def time_norm_small_array_th(self):
        c(np.linalg.norm)(self.array_5)

    def time_det_small_array(self):
        np.linalg.det(self.array_5_5)

    def time_det_small_array_th(self):
        c(np.linalg.det)(self.array_5_5)
...
class Lstsq(Benchmark):
    ...
    def time_numpy_linalg_lstsq_a__b_float64_th(self):
        c(np.linalg.lstsq)(self.a, self.b, rcond=-1)
...
class Einsum(Benchmark):
    ...
    def time_einsum_outer_th(self, dtype):
        c(np.einsum)("i,j", self.one_dim, self.one_dim, optimize=True)
    ...
    def time_einsum_multiply_th(self, dtype):
        c(np.einsum)("..., ...", self.two_dim_small, self.three_dim , optimize=True)
    ...
    def time_einsum_sum_mul_th(self, dtype):
        c(np.einsum)(",i...->", 300, self.three_dim_small, optimize=True)
    ...
    def time_einsum_sum_mul2_th(self, dtype):
        c(np.einsum)("i...,->", self.three_dim_small, 300, optimize=True)
    ...
    def time_einsum_mul_th(self, dtype):
        c(np.einsum)("i,->i", self.one_dim_big, 300, optimize=True)
    ...
    def time_einsum_contig_contig_th(self, dtype):
        c(np.einsum)("ji,i->", self.two_dim, self.one_dim_small, optimize=True)
    ...
    def time_einsum_contig_outstride0_th(self, dtype):
        c(np.einsum)("i->", self.one_dim_big, optimize=True)
    ...
    def time_einsum_noncon_outer_th(self, dtype):
        c(np.einsum)("i,j", self.non_contiguous_dim1, self.non_contiguous_dim1, optimize=True)
    ...
    def time_einsum_noncon_multiply_th(self, dtype):
        c(np.einsum)("..., ...", self.non_contiguous_dim2, self.non_contiguous_dim3, optimize=True)
    ...
    def time_einsum_noncon_sum_mul_th(self, dtype):
        c(np.einsum)(",i...->", 300, self.non_contiguous_dim3, optimize=True)

    # sum and multiply:non_contiguous arrays
    def time_einsum_noncon_sum_mul2(self, dtype):
        np.einsum("i...,->", self.non_contiguous_dim3, 300, optimize=True)

    def time_einsum_noncon_sum_mul2_th(self, dtype):
        c(np.einsum)("i...,->", self.non_contiguous_dim3, 300, optimize=True)
    ...
    def time_einsum_noncon_mul_th(self, dtype):
        c(np.einsum)("i,->i", self.non_contiguous_dim1, 300, optimize=True)
    ...
    def time_einsum_noncon_contig_contig_th(self, dtype):
        c(np.einsum)("ji,i->", self.non_contiguous_dim2, self.non_contiguous_dim1_small, optimize=True)
    ...
    def time_einsum_noncon_contig_outstride0_th(self, dtype):
        c(np.einsum)("i->", self.non_contiguous_dim1, optimize=True)
...
class LinAlgTransposeVdot(Benchmark):
    ...
    def time_transpose_th(self, shape, npdtypes):
        c(np.transpose)(self.xarg)

    def time_vdot(self, shape, npdtypes):
        np.vdot(self.xarg, self.x2arg)

    def time_vdot_th(self, shape, npdtypes):
        c(np.vdot)(self.xarg, self.x2arg)
