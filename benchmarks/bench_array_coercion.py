...
class ArrayCoercionSmall(Benchmark):
    ...
    def time_array_th(self, array_like):
        c(np.array)(array_like)

    def time_array_dtype_not_kwargs(self, array_like):
        np.array(array_like, self.int64)

    def time_array_dtype_not_kwargs_th(self, array_like):
        c(np.array)(array_like, self.int64)

    def time_array_no_copy(self, array_like):
        np.array(array_like, copy=False)

    def time_array_no_copy_th(self, array_like):
        c(np.array)(array_like, copy=False)

    def time_array_subok(self, array_like):
        np.array(array_like, subok=True)

    def time_array_subok_th(self, array_like):
        c(np.array)(array_like, subok=True)

    def time_array_all_kwargs(self, array_like):
        np.array(array_like, dtype=self.int64, copy=False, order="F",
                 subok=False, ndmin=2)

    def time_array_all_kwargs_th(self, array_like):
        c(np.array)(array_like, dtype=self.int64, copy=False, order="F",
                 subok=False, ndmin=2)

    def time_asarray(self, array_like):
        np.asarray(array_like)

    def time_asarray_th(self, array_like):
        c(np.asarray)(array_like)

    def time_asarray_dtype(self, array_like):
        np.array(array_like, dtype=self.int64)

    def time_asarray_dtype_th(self, array_like):
        c(np.array)(array_like, dtype=self.int64)

    def time_asarray_dtype(self, array_like):
        np.array(array_like, dtype=self.int64, order="F")

    def time_asarray_dtype_th(self, array_like):
        c(np.array)(array_like, dtype=self.int64, order="F")

    def time_asanyarray(self, array_like):
        np.asarray(array_like)

    def time_asanyarray_th(self, array_like):
        c(np.asarray)(array_like)

    def time_asanyarray_dtype(self, array_like):
        np.array(array_like, dtype=self.int64)

    def time_asanyarray_dtype_th(self, array_like):
        c(np.array)(array_like, dtype=self.int64)
    ...
    def time_ascontiguousarray_th(self, array_like):
        c(np.ascontiguousarray)(array_like)
